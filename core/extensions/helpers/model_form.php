...
/**
 * Helper to create Forms of a model automatically.
 *
 * @category   KumbiaPHP
 * @package    Helpers
 */
...
{
    /**
     * It generates a form of a model (object) automatically.
     *
     * @param object $model
     * @param string $action
     */
    public static function create($model, $action = '')
    {
        ...
        foreach ($fields as $field) {
            $tipo = trim(preg_replace('/(\(.*\))/', '', $model->_data_type[$field])); //EVERYTHING: collect size and other values
            ...
            switch ($tipo) {
                ...
                case 'year':
                case 'day':
                case 'int unsigned': // Numbers

                    if (strripos($field, '_id', -3)) {
                        echo Form::dbSelect($model_name . '.' . $field, null, null, 'Select', '', $model->$field);
                        break;
                    }
                ...
                default: //text, tinytext, varchar, char, etc. its size will be checked
